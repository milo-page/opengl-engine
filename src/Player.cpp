...
void Player::update_physics(const glm::vec3& input_direction, const glm::vec3& gravity, float delta_time) {
    ...
    velocity += move_speed * input_direction * delta_time;
    ...
    if (vector_length(horizontal_input_dir) > EPSILON) {
    ...
    float horizontal_speed_sq = vector_length(horizontal_velocity);
    // 3. Apply Friction/Damping (if no horizontal input)
    if (vector_length(input_direction) < EPSILON && horizontal_speed_sq > EPSILON && grounded) { // Only apply ground friction if grounded
